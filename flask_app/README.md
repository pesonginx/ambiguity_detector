...
```mermaid
gantt
    title Excel to Index 処理タイムライン（100件のデータの場合）
    dateFormat  ss
    axisFormat  %S秒

    section アップロード
    ファイルアップロード           :a1, 00, 2s
    ファイル保存                   :a2, after a1, 1s

    section 前処理
    ファイル検証（読み込み）       :b1, after a2, 3s
    バリデーションチェック         :b2, after b1, 1s
    UUID生成                       :b3, after b2, 2s
    データクレンジング             :b4, after b3, 1s

    section データ処理
    旧データ削除                   :c1, after b4, 2s
    JSON生成                       :c2, after c1, 3s

    section API処理（順次実行）
    Embedding取得（4スレッド）     :crit, d1, after c2, 25s
    キーワード抽出（4スレッド）    :crit, d2, after d1, 30s

    section 後処理
    ファイル出力                   :e1, after d2, 3s
    インデックス一覧保存           :e2, after e1, 1s
    クリーンアップ                 :e3, after e2, 1s
```

**処理時間の目安**:

| データ件数 | 合計処理時間 | Embedding | キーワード抽出 |
|----------|------------|-----------|---------------|
| 10件 | 約15秒 | 3秒 | 5秒 |
| 50件 | 約30秒 | 12秒 | 15秒 |
| 100件 | 約75秒 | 25秒 | 30秒 |
...
**注意事項**:
- Embedding取得とキーワード抽出は**順次実行**（Embedding完了後にキーワード抽出開始）
- 各処理内では4スレッドで並列処理（複数レコードを同時処理）
- Azure OpenAI APIのレート制限により、大量データの場合は処理時間が延びる可能性あり
- ネットワーク状況により処理時間が変動
